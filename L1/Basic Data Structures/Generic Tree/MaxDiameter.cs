...
public class MaxDiameter
{
    ...
    public static void Main()
    {
        ...
        FindMaxDiameter(node, 0);

        Console.WriteLine("MaxDiameter " + FindMaxDiameter(node, 0));

    }


    public static int FindMaxDiameter(Node node, int depth)
    {
        int maxDiameter = 0;

        foreach (var child in node.Childrens)
        {
            maxDiameter += FindMaxDiameter(child, depth + 1);
        }
