...
public class AreGenericTreeSameMirrorShape
{
    ...
    public static bool AreTreeSameMirrorShape(Node node1, Node node2)
    {
        if (node1.Childrens.Count() != node2.Childrens.Count())
        {
            return false;
        }
        for (int i = 0, j = node1.Childrens.Count() - 1; i < node1.Childrens.Count(); i++, j--)
        {
            var c1 = node1.Childrens.IndexOf(i);
            var c2 = node2.Childrens.IndexOf(j);
