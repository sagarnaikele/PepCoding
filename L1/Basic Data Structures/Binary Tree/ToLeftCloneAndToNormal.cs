...
public static class ToLeftCloneAndToNormal
{
    ...
    public static Node ToLeft(Node node)
    {
        if (node == null)
            return node;

        Node left = ToLeft(node.left);
        Node right = ToLeft(node.right);
