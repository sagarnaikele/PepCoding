...
public class PathToLeafFromRootInRange
{
    public static void Main()
    {
        ...
        PrintPath(tree, "", 0, 100, 150);

    }


    public static void PrintPath(Node node, String path, int sum, int lo, int hi)
    {
        ...
        if (node.left == null && node.right == null && lo < sum + node.data && node.data + sum < hi)
        {
            Console.WriteLine(path + node.data + " ");
        }

        PrintPath(node.left, path + node.data + " ", node.data + sum, lo, hi);
        PrintPath(node.right, path + node.data + " ", node.data + sum, lo, hi);
